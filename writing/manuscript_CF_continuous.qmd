...
author:
  ...
  - name: Taro Mieno
    ...
    affiliations:
      - name: University of Nebraska Lincoln
  - name: Takashi Tanaka
    orcid: 0000-0001-7116-6962
    email: takashi@agro.au.dk
    affiliations:
...
Efficient nitrogen (N) management remains one of the most critical challenges in modern agricultural production. For many crops, nitrogen fertilization is essential for achieving high crop yields, yet excessive or poorly timed applications can lead to economic losses and environmental harm (@fassa2022site). The principle of applying the right amount of nitrogen at the right place and time requires understanding the spatial and temporal variability in crop nitrogen needs. This variability arises from complex interactions among soil nitrogen availability, soil and field characteristics, and weather conditions. Accurately identifying the optimal nitrogen rate for each site and season is therefore fundamental to improving both profitability and sustainability in crop production systems.

On-farm precision experiments (OFPEs) have emerged as a powerful approach to generate the data necessary for site-specific nitrogen management (@bullock2019data; @lacoste2022farm). Through on-farm agronomic trials, researchers can directly observe yield responses to varying nitrogen rates under real-world field conditions, enabling estimation of yield response functions and the estimation of economically optimal nitrogen rates. In recent years, prediction-oriented machine learning (ML) methods have been widely adopted in this context to model yield as a function of nitrogen and environmental covariates (@krause2020random; @barbosa2020modeling; @barbosa2020risk; @gardner2021economic). However, recent evidence suggests that prediction-oriented ML approaches, despite often achieving high yield-prediction accuracy, do not necessarily recover the underlying yield response to nitrogen. @tanaka2024can show that fertilizer rate recommendations derived from predictive ML models are highly sensitive to algorithm choice and covariate selection, leading to substantial variability in estimated optimal application rates. These findings indicate a critical difference between predicting yield levels and estimating the causal yield response that determines economically optimal nitrogen rates, motivating the use of methods that explicitly estimate causal effects of nitrogen on yield.

In the OFPE setting, causal machine learning methods have shown to be promising in the recovery of full yield response curves. @kakimoto2022causal showed that causal forest can estimate heterogeneous treatment effects of an input on yield more accurately than prediction-oriented machined approaches like random forest, boosted forest, and convolutional neural network. However, their application of traditional CF is only limited to a single-year OFPE experiment from a single field because it was developed primarily to estimate the heterogeneous treatment effects of a discrete treatment. In OFPE, there are typically five or six distinctive target experimental rates (see the first panel of @fig-distribution-N). In @kakimoto2022causal, CF estimates the effect of multiple treatments defined as the increase in nitrogen rate from the lowest rate to the other rates (so N − 1 treatments in N rate trials). However, once data from multiple fields with different target rates are combined, applied nitrogen rates become rather continuous as seen in the bottom panel of @fig-distribution-N. While existing prediction-oriented ML approaches—such as direct applications of random forests or neural networks — can naturally handle continuous treatment variable, they tend to fail to capture heterogeneous effects of a treatment variable (@kakimoto2022causal). As multi-year, multi-field OFPE datasets accumulate, pooling data across fields has become both feasible and desirable—it can improve estimation of yield response functions and deliver stronger agronomic and economic insights. It creates a need for methods that can estimate heterogeneous treatment effects of a continuous treatment variable.
...
The objective of this study is to develop and evaluate new causal machine-learning approaches capable of estimating heterogeneous treatment effects for yield response function estimation using data from on-farm trials, where the treatment variable—nitrogen rate—is continuous and randomized. We introduce two methods: a Single-Orthogonalized Neural Network (SO-NN), which isolates the treatment–response component by removing variation in yield explained solely by field conditions, and a semi-parametric Causal Forest (CF) that incorporates smoothing splines to accommodate continuous treatments. Both methods preserve key strengths of causal machine learning by flexibly modeling complex interactions without imposing restrictive functional assumptions. We use a Monte Carlo simulation framework to assess their performances. comparing them with several existing approaches, such as direct applications of random forest and neural networks. 


We find that the SO-NN approach outperforms the non-orthogonalized model, which represents the common practice in yield response function estimation. In contrast, the Double-Orthogonalized approach performs poorly, and its accuracy does not improve even with larger sample sizes, consistent with theoretical expectations. Underscoring the promise of this approach, our extended CF method successfully captures heterogeneous and nonlinear effects of a continuous treatment and accurately estimates site-specific profit-maximizing nitrogen rates with minimal bias. Although the methodological extension proposed here is primarily motivated by the needs of OFPE, it holds potential for a wide range of applications in applied economics where continuous treatments and heterogeneous effects are of interest.
...
The standard approach in the precision-agriculture literature for estimating site-specific input response is to model the outcome level directly as a function of the treatment/input and covariates—e.g., estimate $Y=f(I,C)$ where $I$ is the input (like nitrogen rate) and $C$ are field characteristics, and then derive optimal input decisions from that estimated function. This is what is typically done in the recent on-farm precision experimentation literature (e.g., @barbosa2020risk; @barbosa2020modeling; @krause2020random; @gardner2021economic). The problem is that these methods optimize prediction of yield levels—not the causal effect of changing the input. Good level prediction at the observed input does not guarantee accurate recovery of the response function needed for optimal input choice, because the fitted $f(I,C)$ can confound the effect of the input with other factors that also affect yield.

To identify causal effects rather than just predict levels in the single binary treatment case, researchers use double/debiased machine learning (DML), which introduces orthogonalization so the treatment-effect estimator is insensitive to small errors in the auxiliary fits (@chernozhukov2018double; @mackey2018orthogonal). Consider the structural model (Model 1):

$$
Y=\theta(X) T+g(X, W)+v,
$$

where $Y$ is the outcome, $T \in\{0,1\}$ is the treatment, vector $X$ contains the covariates that also modify the treatment effect $\theta(X)$, vector $W$ contains additional predictors of $Y$, and $v$ is noise. Orthogonalization estimates $f(X, W)=\mathbb{E}[Y \mid X, W]$ and $h(X, W)=\mathbb{E}[T \mid X, W]$, forms residuals $Y_{\mathrm{res}}=Y-f(X,W)$ and $T_{\mathrm{res}}=T-h(X, W)$, and then relates  $Y_{\mathrm{res}}$ to $T_{\mathrm{res}}$. Because this score is Neyman-orthogonal and is typically implemented with cross-fitting, small errors in $f$ and $h$ do not induce first-order bias in $\theta(X)$ [(@chernozhukov2018double].
...
Thus regressing $Y_{\mathrm{res}}$ on $T_{\mathrm{res}}$ identifies $\theta(X)$ with the remaining variation captured by the error term $v$. 

@kakimoto2022causal applied this framework to estimate heterogeneous treatment effects of nitrogen on yield. However, when data are pooled across fields and applied nitrogen rates become continuous, the standard residualization of the treatment in DML does not extend cleanly to that setting.
...
and the clean decomposition that underlies double-orthogonalization in the linear-in-$T$ case breaks down. Instead, we apply single-orthogonalization (SO). We first estimate $f(X, W)=\mathbb{E}[Y \mid X, W]$ and form the residual $Y_{\mathrm{res}}=Y-f(X, W)$, and then recover $\theta(X, T)$ by flexibly modeling  $Y_{\mathrm{res}}$ as a function of $(X,T)$. This approach removes baseline variation due to $g(X,W)$ while preserving the full interaction structure between $X$ and the continuous treatment.

::: {.column-margin}
[We will add justifications for this action later.]{style="color: blue;"}
:::
...
In the single-orthogonalized framework only the baseline component $g(X,W)$ is partialed out before the final-stage estimation; the continuous treatment $N$ and heterogeneity-inducing covariates $X$ remain in their original form. Formally, starting from Model 3, we first estimate $g(X,W)=\mathbb{E}[y \mid X,W]$ using a cross-fitted random forest and form the residualized outcome

$$
\begin{align*}
\tilde{y}=y-\widehat{g}(X,W).
\end{align*}
$$
...
\end{align*}
$$
where $P_{\text {Corn }}$ is the corn price, $P_{\text {N }}$ is the nitrogen price, and $\hat{f}$ is the estimated yield response function.

### Double-orthogonalized CF (DO-CF) and NN (DO-NN) 

#### DO‑CF (continuous)

To adapt double-orthogonalization to Model 3, we estimate the heterogeneous response $f(X,N)$ by expanding the continuous treatment $N$ in a set of spline basis functions and allowing their coefficients to vary with the heterogeneity-inducing covariates $X$.

In spline regression, a single variable can be decomposed into multiple basis functions in a linear-in-parameter manner. For example, consider the following general model, where the non-linear impact of $T$ is represented by $f(T)$:

$$
y = f(T) + v. 
$$

Non-linearity in $f(T)$ can be captured by a linear combination of basis functions as follows:

$$
f(T) = \alpha_1 \cdot S_1(T) + \alpha_2 \cdot S_2(T) \dots, + \alpha_K \cdot S_K(T) = \sum_{k=1}^K \alpha_k \cdot S_k(T),
$$

where $\alpha_k$ is the coefficient to be estimated and $S_k(T)$ is the value of the basis function $S_k(\cdot)$ evaluated at $T$. By changing the value of $\alpha_k$, $\sum_{k=1}^K \alpha_k S_k(T)$ can flexibly approximate various non-linear functions that cannot be captured by commonly used functional forms. In spline regression, the weights (or coefficients) are obtained by regression with the spline bases included as covariates.

```{r fig-spline-examples, fig.cap="An example of spline regression fit of the yield response curved and weighted basis functions"}
knitr::include_graphics(here::here("writing/Figures/g_tsp_fit.pdf"))
```

@fig-spline-examples shows the fitted line (red line), which is the sum of the six weighted (multiplied by coefficients) thin-plate splines (dotted lines). As can be seen, the fitted line closely traces the true yield response curve (blue line).

Since this modeling approach is linear-in-parameters, it can be readily used inside the CF framework. Instead of a constant as the weight (coefficient) for each spline basis function, the weight is made a function of observables, $X$, as follows:

$$
\begin{align*}
y=\sum_{k=1}^K f_k(X) S_k(N)+g(X, W)+v.
\end{align*}
$$

Although double-orthogonalization generally fails when $f(X,N)$ is an arbitrary nonlinear function of $N$, this spline-based formulation restores the key linear-in-parameter structure that makes DO valid, because the model is linear in the transformed treatment basis $\left\{S_k(N)\right\}$. Residualizing each basis component and the outcome with respect to $(X,W)$ then yields a clean decomposition analogous to the linear-in-$T$ case. 

Let $\widetilde{S}_k(N)=S_k(N)-\mathbb{E}\left[S_k(N) \mid X, W\right]$. Then

$$
\begin{align*}
\tilde{y}=y-\mathbb{E}[y \mid X, W]=\sum_{k=1}^K f_k(X) \widetilde{S}_k(N)+v,
\end{align*}
$$

so causal forest can nonparametrically estimate each heterogeneous weight $f_k(X)$  by relating the residualized outcome to the residualized spline bases. This Neyman-orthogonal (R-learner) style decomposition preserves the ability to capture complex, heterogeneous, and nonlinear treatment effects while inheriting the robustness of double-orthogonalization: small errors in the nuisance fits for $g(X,W)$ or the conditional means of the bases do not induce first-order bias in $f_k(X)$. The estimated $f_k(X)$ then combine to recover the full heterogeneous response function and, by differencing across candidate $N$ values, site-specific treatment effects and economically optimal nitrogen rates.

#### DO‑NN

In the DO-NN approach, we apply double-orthogonalization to Model 3 by residualizing both the outcome $y$ and the continuous treatment $N$ with respect to $(X, W)$ using cross-fitted random forests:
...
\end{align*}
$$
To assess the economic relevance of these modeling approaches, we use profit loss relative to the true site-specific EONR. The average per-acre profit from true site-specific EONR (denoted as $\pi_{true}$) is
...
\end{align}
$$
Where $P_c$ and $P_N$ represent the prices of corn yield and nitrogen, respectively.

The average profit per acre associate with the site-specific N prescription by each model $m$ (denoted as $\pi$) is defined as
...
The profit loss of model $m$ relative to the true maximum profit achievable is then defined as
...
#   geom_sf(data = filter(subplot_sf, buffer == 1), fill = "yellow", alpha = 0.4) +
#   geom_sf(
#     data = filter(subplot_sf, buffer == 0),
#     linewidth = 1.2,
#     fill = NA,
#     color = "red"
#   ) +
#   
#   # keep buffer outline separate
#   geom_sf(
#     data = filter(subplot_sf, buffer == 1),
#     linewidth = 1.0,
#     fill = NA,
#     color = "black"
#   ) +
...
# all_data <- readRDS(
#   here::here("data", "raw_sim_data.rds")
# )
# 
# trial_design <- data.table::as.data.table(
#   all_data$trial_design[[1]]
# )[sim == 1]
# 
# field <- data.table::as.data.table(
#   all_data$field_sf[[1]]
# )
# 
# td_sf <- trial_design[
#   field,
#   on = .(block_id, plot_in_block_id)
# ] |>
#   sf::st_as_sf()
# 
# g_td <- ggplot2::ggplot(td_sf) +
#   ggplot2::geom_sf(
#     ggplot2::aes(fill = factor(N_tgt)),
#     color = NA
#   ) +
#   ggplot2::scale_fill_viridis_d(
#     name = "Nitrogen Rate (lb/acre)"
#   ) +
#   ggplot2::theme_void() +
#   ggplot2::theme(
#     legend.position = "bottom"
...
# ggplot2::ggsave(
#   filename = here::here("writing", "Figures", "g_td.pdf"),
#   plot = g_td,
#   width = 6.5,
#   height = 5
# )
...
here::here("writing", "Figures", "g_td.pdf")
)
```
...
We compare six estimators:  CF, SO_NN, DO_ANN, NO_ANN, SO_RF, and NO_RF over 500 Monte-Carlo replications and five training sizes (1, 3, 5, 10 and 20 fields). Each replication contains 1,440 evaluation units. Accuracy is assessed by the RMSE of the estimated economically optimal nitrogen rate (EONR) with respect to the true (simulated) EONR.
...
Taken together, @fig-rmse-hist and @fig-avg-performance illustrate clear performance patterns across methods and training sizes. CF delivers low and tightly concentrated RMSE distributions even with limited data—consistently maintaining strong accuracy and stability for 1–5 fields (mean RMSE ≈ 14.7, 13.1, and 12.1, respectively), with only modest additional gains at 10 and 20 fields (11.9–11.8). SO_NN initially lags behind CF (18.4 at one field), but its mean RMSE decreases steadily as more fields are added, surpassing CF performance at 10 and 20 fields (11.2 and 8.5, respectively). This pattern hilightes the neural network’s ability to exploit richer spatial and agronomic heterogeneity when sufficient training data are available.

By contrast, NO_ANN exhibits substantial instability in the single-field case, with a heavy right tail and much higher errors, but rapidly converges toward the leading methods by 5–10 fields (mean RMSE improving to around 14–13), highlighting its strong dependence on sample size. SO_RF improves modestly with more fields (mid-20s RMSE for 10–20 fields) but remains consistently less accurate than SO_NN or CF. NO_RF maintains broad, right-shifted distributions and higher mean errors throughout (≈ 36–40), confirming its role as a weak baseline. Finally, DO_ANN performs poorly throughout, with both histograms and means showing persistently high errors (≈ 90+) and negligible benefit from additional data, showing that the orthogonalization step breaks the original relationships in the data instead of preserving them.
...
# ggsave(
#   filename = here::here("presentation/proposal/fig_rmse_hist.png"),
#   plot = p,
#   width = 12,
#   height = 7,
#   bg = "white"
...
rmse_summary <- rmse_df %>%
  dplyr::group_by(Model_lab, field_f) %>%
  dplyr::summarise(
    n      = dplyr::n(),
...
# 
p_mean <- ggplot(rmse_summary, aes(x = field_f, y = mean,
                                   group = Model_lab, color = Model_lab)) +
...
# ggsave(here::here("presentation", "proposal", "model_improvement.png"), p_mean, width = 9, height = 5, dpi = 300)
```
...
@fig-improvement compares the performance of CF and SO_NN relative to the worst-performing model, DO_ANN, across different numbers of fields. Both methods deliver large improvements in RMSE at all sample sizes. With only one field, CF and SO_NN reduce error by roughly 80–84%. As more fields become available, both approaches strengthen further: CF’s improvement rises to about 87% by 5–10 fields, while SO_NN continues to gain with increasing data, reaching nearly 91% at 20 fields.
...
models_to_keep <- c("SO_NN", "CF")

avg_by_filtered <- avg_by %>%
  dplyr::filter(Model %in% models_to_keep | Model == baseline)

# baseline per field 
baseline_by_field <- avg_by_filtered  %>%
...
best_models <- rmse_summary %>%
  ...
  select(Model= Model_lab, field_f, mean)

best_table <- best_models %>%
  gt() %>%
  tab_header(
...
#gtsave(best_table, here::here("Proposal_presentation/tbl_best_model_by_field_count.png"))
...
source(here("codes", "R", "visualize_yield_response_function.R"))
#p4 <- plot_yield_response(model = "SO_NN", n_fields = 10)
...
source(here("codes", "R", "visualize_yield_response_function.R"))
#p5 <- plot_yield_response(model = "SO_NN", n_fields = 20)
...
# #| fig-cap: "Yield response functions for CF and SO_NN models across different field counts."
# #| echo: false
# #| fig-width: 10
# #| fig-height: 8
# 
...
@fig-profit-loss shows the best-performing model at each training-data size based on average profit loss relative to the true economically optimal nitrogen rate. When only 1–5 fields are available, the semi-parametric-CF achieves the lowest profit loss. As the number of training fields increases, the SO_NN becomes the top performer, attaining substantially lower profit loss at 10 and 20 fields. This pattern shows that CF works best when data are limited, while SO-NN performs better once more fields are available.

```{r}
#| label: fig-profit-loss
#| fig-cap: "Profit loss"
knitr::include_graphics("Figures/best_models.png")
```
...
For the SO_NN, shown for 10 and 20 fields, the estimated response matches the true curve more closely in both slope and curvature, particularly at higher nitrogen levels where CF tended to flatten. At 20 fields, SO_NN produces the most accurate approximation overall, with prediction and truth nearly coincident across the nitrogen range. These differences visually reinforce the earlier quantitative results: CF offers strong robustness with sparse multi-field data, while SO_NN increasingly excels as richer spatial heterogeneity becomes available, enabling superior recovery of the underlying agronomic yield response function.
...
title_SO <- grid::textGrob(
  "Single-Orthogonalized ANN (SO_NN): 10 and 20 fields",
  gp = grid::gpar(fontsize = 13, fontface = "bold")
)
...
This study evaluates alternative causal-ML strategies for estimating heterogeneous yield response to a continuous nitrogen treatment in an OFPE setting. CF consistently attains low and tightly concentrated EONR errors with only 1–5 training fields, indicating strong small-sample stability (see @fig-rmse-hist and @fig-avg-performance). In contrast, SO_NN improves monotonically with additional fields and ultimately dominates once ≥10 fields are available, achieving the lowest mean RMSE overall (e.g., 11.2 at 10 fields and 8.5 at 20 fields).

This pattern is consistent with the structural differences between the two approaches. CF relies on orthogonalized residualization, which reduces sensitivity to first-stage estimation error and yields robustness in data-constrained settings. SO_NN, by contrast, does not employ double orthogonalization and is therefore more reliant on accurate function approximation of the outcome–treatment relationship. As the number of training fields increases, the richer data environment mitigates this limitation, allowing SO_NN to benefit from its greater representational flexibility and outperform CF in larger-sample settings. Taken together, these complementary strengths suggest that CF is preferred under data-constrained conditions, whereas SO_NN becomes the method of choice as multi-field datasets accumulate.
