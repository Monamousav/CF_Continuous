...
::: {.column-margin}
<span style = "color: blue;"> We will add justifications for this action later. </span>
:::
...
@fig-spline-examples shows the fitted line (red line), which is the sum of the six weighted (multiplied by coefficients) thin-plate splines (dotted lines). As can be seen, the fitted line closely traces the the true yield response function (blue line).

Since this modeling approach is linear-in-parameter, it can be readily used inside the CF framework. Instead of a constant as the weight (coefficient) for each spline basis function, the weight is made a function of observables, $X$, as follows:
...
Taken together, @fig-rmse-hist and @fig-avg-performance illustrate clear performance patterns across methods and training sizes. CF delivers low and tightly concentrated RMSE distributions even with limited data—consistently maintaining strong accuracy and stability for 1–5 fields (mean RMSE ≈ 14.7, 13.1, and 12.1, respectively), with only modest additional gains at 10 and 20 fields (11.9–11.8). SO_ANN initially lags behind CF (18.4 at one field), but its mean RMSE decreases steadily as more fields are added, surpassing CF performance at 10 and 20 fields (11.2 and 8.5, respectively). This pattern hilightes the neural network’s ability to exploit richer spatial and agronomic heterogeneity when sufficient training data are available.
...
@fig-improvement compares the performance of CF and SO_ANN relative to the worst-performing model, DO_ANN, across different numbers of fields. Both methods deliver large improvements in RMSE at all sample sizes. With only one field, CF and SO_ANN reduce error by roughly 80–84%. As more fields become available, both approaches strengthen further: CF’s improvement rises to about 87% by 5–10 fields, while SO_ANN continues to gain with increasing data, reaching nearly 91% at 20 fields.
