...
for (i in seq_len(1)) {
  ...
  tryCatch({
    ...
    cf_fit <- causal_forest(
      X = X,
      Y = Y,
      W = T_info$T_sp,     # spline
      num.trees = 2000,
      min.node.size = 10,
      seed = 3747823
    )

    # Estimate treatment effects for test data
    te_hat_cf <- predict(cf_fit, newdata = X_test)$predictions

    
    te_info <- data.table(aunit_id = test_data$aunit_id, est = te_hat_cf)
    ...
    ## 8) Save results object (optional)
    # res <- list(
    #   cf_fit        = cf_fit,
    #   te_info       = te_info,
    #   T_seq         = T_seq,
    ...
    message("Finished sim ", test_sim_id,
            " | corr = ", round(corr_val, 3))

    # Free RAM
    rm(cf_fit, te_info, response_data, train_data, test_data, e)
    if (i %% 10 == 0) gc()
  },
  error = function(e) {
    message("ERROR in sim ", test_sim_id, ": ", conditionMessage(e))
