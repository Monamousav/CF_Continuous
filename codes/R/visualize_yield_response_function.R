...
# # fields_label <- function(n_fields) {
# #   if (n_fields == 1) "one_field"
# #   else if (n_fields == 3) "three_fields"
# #   else if (n_fields == 5) "five_fields"
# #   else if (n_fields == 10) "ten_fields"
# #   else if (n_fields == 20) "twenty_fields"
# #   else paste0(n_fields, "_fields")
# # }
# 
# fields_label <- function(n_fields, model = "SO_ANN") {
#   # normalize input
#   n_str <- tolower(as.character(n_fields))
#   n_str <- stringr::str_replace_all(n_str, "_fields?$", "")
# 
#   # map word → numeric
#   lookup <- c(
#     "one" = 1, "1" = 1,
...
#   )
#   n_val <- lookup[[n_str]]
#   if (is.null(n_val)) stop("Unknown n_fields value: ", n_fields, call. = FALSE)
# 
#   path1 <- paste0(n_val, "_fields")
#   path2 <- switch(as.character(n_val),
#                   "1"  = "one_field",
#                   "3"  = "three_fields",
#                   "5"  = "five_fields",
#                   "10" = "ten_fields",
#                   "20" = "twenty_fields")
# 
#   # --- check which folder actually exists (for any model) ---
#   base_dir <- here::here("results", "yield_response_function_for_one_iteration")
#   path_num  <- file.path(base_dir, paste0("YRF_", model, "_", path1))
#   path_word <- file.path(base_dir, paste0("YRF_", model, "_", path2))
# 
#   if (dir.exists(path_num)) {
#     return(path1)
#   } else if (dir.exists(path_word)) {
#     return(path2)
#   } else {
#     # if neither exists, return numeric form by default
#     warning("No folder found for either naming style; returning numeric form.")
#     return(path1)
#   }
# }
# 
...
# results_csv_path <- function(model, n_fields, test_id = 1,
#                              base_dir = here::here("results", "yield_response_function_for_one_iteration")) {
#   label <- fields_label(n_fields, model)
#   subdir <- paste0("YRF_", model, "_", label)
#   file.path(base_dir, subdir, sprintf("yield_response_%s.csv", test_id))
# }
...
####################### Update for manuscript ##################################

# ============================================================
# visualize_yield_response_function.R
# ============================================================
...
# ---------- Compute global x/y limits across multiple models ----------
compute_global_limits <- function(models,
                                  n_fields,
                                  test_id = 1,
                                  base_dir = here("results", "yield_response_function_for_one_iteration")) {
...
plot_yield_response <- function(model = "NO_ANN",
  ...
  # Optional title
  if (add_title) {
    p <- p + ggtitle(paste0("Yield response (", model, ": ", n_fields, "_fields)"))
  } else {
    p <- p + ggtitle(NULL)
  }
