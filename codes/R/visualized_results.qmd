...
models_to_compare <- c("NO_ANN", "DO_ANN", "SO_ANN", "NO_RF", "SO_RF", "CF")

# 
plot_yield_response("NO_ANN", 1, global_models = models_to_compare)
plot_yield_response("NO_ANN", 3, global_models = models_to_compare)
...
plot_yield_response("NO_ANN", 20, global_models = models_to_compare)
plot_yield_response("DO_ANN", 1, global_models = models_to_compare)
plot_yield_response("DO_ANN", 3, global_models = models_to_compare)
...
plot_yield_response("DO_ANN", 20, global_models = models_to_compare)
plot_yield_response("SO_ANN", 1, global_models = models_to_compare)
plot_yield_response("SO_ANN", 3, global_models = models_to_compare)
...
plot_yield_response("SO_ANN", 20, global_models = models_to_compare)
plot_yield_response("NO_RF",  1, global_models = models_to_compare)
plot_yield_response("NO_RF",  3, global_models = models_to_compare)
plot_yield_response("NO_RF",  5, global_models = models_to_compare)
plot_yield_response("NO_RF",  10, global_models = models_to_compare)
plot_yield_response("NO_RF",  20, global_models = models_to_compare)
plot_yield_response("SO_RF",  1, global_models = models_to_compare)
plot_yield_response("SO_RF",  3, global_models = models_to_compare)
plot_yield_response("SO_RF",  5, global_models = models_to_compare)
plot_yield_response("SO_RF",  10, global_models = models_to_compare)
plot_yield_response("SO_RF",  20, global_models = models_to_compare)
plot_yield_response("CF", 1, global_models = models_to_compare)
plot_yield_response("CF", 3, global_models = models_to_compare)
plot_yield_response("CF", 5, global_models = models_to_compare)
plot_yield_response("CF", 10, global_models = models_to_compare)
plot_yield_response("CF", 20, global_models = models_to_compare)
...
parse_n_fields <- function(folder) {
  ...
  # text pattern
  words <- c("one"=1,"three"=3,"five"=5,"ten"=10,"twenty"=20)
  word <- str_match(f, "(one|three|five|ten|twenty)_?fields?")[,2]
...
read_eonr_file <- function(f) {
  obj <- read_any(f)
  if (is.data.frame(obj) && all(c("opt_N_hat","opt_N") %in% names(obj))) {
    list(pred = as.numeric(obj$opt_N_hat), true = as.numeric(obj$opt_N))
...
build_rmse_table <- function(base_dir = here("results"),
  ...
  dirs <- fs::dir_ls(base_dir, type = "directory", recurse = FALSE) %>%
    keep(~ str_detect(basename(.x),
                      regex("^(cf|cf_outcome|rann|half_rann|simple_ann|no_rf|halfrrf)",
                            ignore_case = TRUE)))
  ...
  map_dfr(dirs, function(dir_path) {
    ...
    # only keep 1, 3, 5, 10, 20
    if (!n_f %in% c(1, 3, 5, 10, 20)) return(NULL)
...
rmse_tbl_raw <- readRDS(here("results", "rmse_tbl_raw.rds"))
...
# -------------------------
# 2) RMSE dataset (one per Model, field, sim)
# -------------------------
rmse_df <- rmse_tbl_raw %>%
  group_by(Model, `#field`, sim) %>%
...
p_box <- ggplot(rmse_df, aes(x = Model_lab, y = RMSE)) +
  geom_boxplot(fill = "skyblue3", alpha = 0.8, outlier.size = 0.8) +
  facet_wrap(~ field_f, nrow = 1) +
  labs(
    x = "Model",
    y = "RMSE"
  ) +
...
# ggsave(
#   here::here("presentation", "proposal", "p_box.png"),
#   p_box,
#   width = 14,
#   height = 16,
#   dpi = 300
