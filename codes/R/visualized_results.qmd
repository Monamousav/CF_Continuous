...
parse_model <- function(folder) {
  ...
  case_when(
    str_detect(f, "^cf_python")     ~ "CF_Python",
    str_detect(f, "^cf")            ~ "CF",
...
read_eonr_file <- function(f) {
  ...
  if (is.data.frame(obj) && all(c("opt_N_hat","opt_N") %in% names(obj))) {
    list(pred = as.numeric(obj$opt_N_hat), true = as.numeric(obj$opt_N))  # CF or CF_python csv
...
build_rmse_table <- function(base_dir = here("results"),
  ...
  dirs <- fs::dir_ls(base_dir, type = "directory", recurse = FALSE) %>%
    keep(~ str_detect(basename(.x),
                      regex("^(cf|cf_python|rann|half_rann|simple_ann|no_rf|halfrrf)",
                            ignore_case = TRUE)))
