---
title: "Chapter 1: A new model selection approach based on local economically optimal input rate"
author:
  - name: Taro Mieno$^*$
  - name: Mona Mousavi$^*$
...
+ In all of the previous studies, the best model was selected based on their abilities to predict \textcolor{blue}{yield level} at the observation points (yield-based model selection).  
...
Steps for a single iteration:
  ...
  + Step 3: Select the best model among the 5 candidate models based on
...
linear model, linear spatial error (SE) model, random forest (RF), boosted regression forest (BRF), and causal forest (CF).  
...
+ One possible scenario of the spatial variability in the field characteristics is shown.

+ This study assumed the yield data were collected at the subplot level (total 1440 subplots).


## The simulated OFPE field:
```{r}
#| echo: false
#| fig-align: "center"
#| out-width: "95%"
knitr::include_graphics("~/Desktop/g_field_map.png")
```
...
The true economically optimal N rate for each cell is then determined by:
...
+ $p$: the crop price, 
+ $w$ the nitrogen fertilizer price, 
+ $f_{j, i}(N)$: previously defined true yield response function for the specific cell.
...
+ six N trial rates ($\{80, 130, 154, 184, 219, 270\}$ kg/ha.) followed a 6 × 6 Latin square design.
...
#| fig-align: "center"
#| out-width: "100%"
knitr::include_graphics("g_N.png")
```
...
#| fig-align: "center"
#| out-width: "100%"
knitr::include_graphics("/Users/monamousavi/Dropbox/Causal_climate/g_y.png")
```
...
+ The true response parameters $\left(\alpha_{j, i}, \beta_{j, i}, \gamma_{j, i}, \tau_{j, i}, \eta_{j,i}\right)$ and economically optimal N rates $N_{j, i}^*$ are not directly observable by producers or researchers.
...
$$\beta = \beta^1 + \beta^2, \;\;
\beta^1 = \beta^{1,1} + \beta^{1,2}, \;\;\beta^2 = \beta^{2,1} + \beta^{2,2}$$ 
$$\tau = \tau^1 + \tau^2, \;\;
\tau^1 = \tau^{1,1} + \tau^{1,2}, \;\;\tau^2 = \tau^{2,1} + \tau^{2,2}$$ 
$$
\eta = \min(\eta^1, \eta^2), \;\;
\eta^1 = \min(\eta^{1,1}, \eta^{1,2}), \;\;
\eta^2 = \min(\eta^{2,1}, \eta^{2,2})
$$

# Identifying the true best model among the five candidate models 

+ Each of the candidate models are trained using the whole experiment data and then variable rate EONR are estimated for the whole field. 

+ The estimated variable rate N recommendations are evaluated against the true variable rate N to evaluate the performance of the models (using profit loss relative to the true site-specific EONR as the criteria):
...
## An example of the train-test split for each of the five folds
...
+ Steps for each fold and candidate models:
  ...
  5. Calcurate the average of the site-specific EONR ($\hat{N}^*_{f,m}$)
  6. Calcurate the RMSE of the estimated local EONR:
...
## Simulation Results: Accuracy of the model selection approaches

```{r}
#| echo: false
#| results: 'asis'
tbl <- readRDS("main_selection_accuracy_table.rds")
invisible(flextable::save_as_image(tbl, path = "table_accuracy.png"))
knitr::include_graphics("table_accuracy.png")
...
### Causal methods for Site-specific yield response function estimation
...
### Double-ML (binary treatment ($T$) case):

DML works fine in a \textcolor{blue}{binary} treatment ($T$) case (model assumes linearity in $T$):
...
& Y_{res}= \\
& \begin{aligned}
\theta(X) \cdot T + g(X, W) + v 
- & \underbrace{E\{[\theta(x) \cdot T + g(X, W) + v] \mid X, W\}}= \\
& \textcolor{blue}{=} \textcolor{blue}{\theta(X) \cdot E(T \mid X, W) + g(X, W)} \\
& \textcolor{blue}{=}  \textcolor{blue}{\theta(X) \cdot h(X, W) + g(X, W)}
\end{aligned} \\
& \theta(X) \cdot T + g(X, W) + v 
- \theta(X) \cdot h(X, W) - g(X, W)= \\
& \theta(X) \cdot [\underbrace{T - h(X, W)}_{T_{res}}] + v \\
& \Rightarrow Y_{res} = \theta(X) \cdot T_{res} + v
\end{aligned}
$$
...
### Double-ML (continuous treatment ($T$) case):


DML fails in a \textcolor{blue}{continuous} treatment ($T$) case:
